...
def _batchify_batching_rule(inputs, batch_axes):
    state, like = inputs
    state_batch_axis, like_batch_axis = batch_axes
    assert state_batch_axis is batching.not_mapped
    assert like_batch_axis is not batching.not_mapped
    return state, like_batch_axis


_batchify_p = jax.core.Primitive("batchify")
_batchify_p.def_impl(lambda state, like: state)
_batchify_p.def_abstract_eval(
    lambda state, like: jax.ShapedArray(like.shape, like.dtype)
)
batching.primitive_batchers[_batchify_p] = _batchify_batching_rule
xla.register_translation(
    _batchify_p,
    xla.lower_fun(lambda state, like: state, multiple_results=False, new_style=True),
)
...
def get_state(index: StateIndex, like: PyTree[Array]) -> PyTree[Array]:
    ...
    **Raises:**
    ...
    # (inference=True vs inference=False) x (JIT vs non-JIT)
    #
    # First, the inference=False x (JIT vs non-JIT) case.
    #
    ...
    if index.inference:
        try:
            state, _, version = _state_cache[index._obj]
        except KeyError as e:
            raise KeyError("Cannot get state before it has been set") from e
        if version == index._version.value:
            # Maybe in JIT.
            state = index._state
        else:
            # Definitely not in JIT.
            currently_jitting = isinstance(jnp.array(1) + 1, jax.core.Tracer)
            assert not currently_jitting
            state = jax.tree_map(jnp.asarray, state)
        return _batchify_p.bind(state, like)
    else:
        _monkey_patch()
